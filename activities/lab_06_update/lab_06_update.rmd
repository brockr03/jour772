...
```{r}
baltimore_911 <- read_csv("/Users/rebeccaheimbrock/Desktop/jour772/activities/lab_06_update/data/baltimore_911.csv") %>% 
  clean_names() %>% 
  mutate(across(zip_code, as.character))
```
```{r}
#Raphael helped me with this code
...
**Write the answer:**Carroll, Cecil, Allegany
...
**Write answer here:** Cecil county had the most calls in August, and it seems like May was a high call-volume month for at least three counties. It's interesting that Alleghaney had 17 calls in November, as I was under the impression that holiday months were typically worse for ODs.

Q2b: Use "combined_911" data and write code to calculate the most calls per county and location. 

```{r}
total_info <- combined_911 |> 
  group_by(county) |> 
...
**Write answer here:** Carroll had the most amount of calls, their total call number is 448
...
```{r}
#Note that Raphael helped me with this code.
...
**Write the answer here:** The zip codes are overwhelmingly on the northern, western and northwestern side of Baltimore. The median individual income (according to census data) in zip 21217, where a notably large amount of calls are recorded, is just below $30,000. This area is also known as Upton. According to Wikipedia, Upton was one of the most affluent Black neighborhoods in the United States. Wikipedia speculates that more affluent Black families eventually left for the suburbs, contributing to the ultimnte decline of the area.
...
**Write the answer here:** The zip code with the highest rate of calls per 1,000 people is 21217. As I noted above, this is an interesting area for a number of reasons. As a location that was once the site of prosperity for Black families, it has seemingly declined due to both flight to the suburbs and red lining policies that push out and impoverish Black people. This, for me, was very emblematic of Christina Sharpe's notion of 'the wake' of chattel slavery. The zip code with the second highest number of calls is home to Poe Baltimore, the site of the Edgar Allen Poe house and museum. 
