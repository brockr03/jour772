...
**Answer** It simplified our dataset, creating code that allows the new simplified data to appear as its own clickable 'thing' in the Data window on the right. 
...
**Answer:** Now, under the expenditure_date column it says "<date>" instead of "<chr>". The dates are also formatted differently. Before, the dates were formatted "M/D/Y" but now they are formatted "Y-M-D"; I'm assuming the format is what helps allow the program to recognize these as dates and not characters.
...
**Answer** Now the values are grouped by the first of each month, showing the total amount of donations accrued by month, as opposed to each day of the month.
...
**Answer** This code is taking data and lumping it together in a new column based on specific language -- "cox" "moore" etc -- which we can then use to decipher how much Moore won by.
...
**Answer** I'm not sure, I don't see a difference. I'm wondering what I did wrong ... I will check in about this part on Monday.
...
**Answer** This answer is actually less useful, in my opinion, because it's listed with the lowest moore percentage first. I would have preferred to see the highest listed in descending order. But if you wanted to write an article about which places had the LEAST pct of votes fo more, I'm sure this would be useful.
...
**Answer** According to stringer.tidyverse.org --NOT chatgpt-- str_to_lower converts to lower case, and str_to_sentence converts to sentence case, meaning the first letter of each sentence is capitalized. These are similar in the sense that they standardize things based on capitalization and sentence structure.
...
**Answer** The mutate statement is combining states by full name and abbreviation. 
...
**Answer** It does make sense, now there is no difference between Maryland and MD, so we can clearly see the exact amount spent per state.
...
1.  Can we use the floor-date function to filter Maryland expenditure data by month, in order to answer how much was spent per month?

2.  Can we standardize the UMD course data to combine courses listed multiple times but with different names?

3.  Can we use the better percentage calculation to figure out what percentage of courses have had a decrease in seat availability throughout, say, the recent 10 years?
