...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** I don't see any parsing errors when I run it.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** Vendor Name data is chr, Amount is dbl, and Date is also chr.
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer** 60 rows of zip codes have only four characters. As discussed in class, this is because the zip codes begin with '0' and arent recognized by the coding system.
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer** The NA values seem potentially problematic.
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer** I see grantee, zip code, fiscal year, amount, description, category, fiscal period, and date data. I think the $60 grant to Land Preservation Trust is potentially suspicious?
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer** The headers now have consistent capitalization, zip codes look good.
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer** `the first line labels the new table as "cleaned_md_grants_loans." Then, the clean_names function allows for the grantor source to be sorted alongside the new column name 'grantee.'
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** 58 dupes are possible.
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer** 19,453
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** Now there is a new column with simplified zip codes.
