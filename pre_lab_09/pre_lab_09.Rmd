...
md_gov_county_1 <- md_gov_county %>%
  mutate(
    total_votes = Cox + Moore + Lashar + Wallace + Harding + `Write-ins`,
    pctmoore = Moore / total_votes
  )

write_csv(md_gov_county_1, "data/md_gov_county_1.csv")
```
...
**Write Your Answer Here, use complete sentences** Describe the changes from the previous map
*Answer*: In the previous map, there were no values or really any descriptive markers to help demonstrate what data was being represented in the map, and what that data meant. The map looked was color-coded, but that was it. When I hovered over each county, I saw a zip code and percentage -- again with no explanation. After the edits, I have a title that -- while maybe too generic -- clearly communicates that we are looking at a map of which counties had what percent of votes for Moore. I also have a detailed description for people who use screen readers. Even better, now when I hover over each county I can see the name of the county AND how many votes for Moore were in said county. I also changed the color coding so that it clearly demonstrates the strong swing for Moore in PG County and Baltimore City. 
...
**Answer Here** Put the URL of your map here. https://datawrapper.dwcdn.net/DE4qb/1/
