...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** Object ID, Zip Code, Shape length, shape area, geometry, zip name, and something called 'existing.'
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** The zip code, name of the city/region associated with the zip codes, geometry, shape length, and object ID.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This is a comprehensive map of the different zip code areas in the state of MD.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now two libraries are installed.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We have a lot of data about states and counties, including name and GEOID.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We now have a spreadsheet with every county name, plus zip code information.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This is a nice, clean map of Maryland's counties.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This is a map of counties and zip code areas. It's a little too busy for my taste. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This is a spreadsheet containing foreclosures in March by zip code.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We have a chart with much more data that shows foreclosures by zip code and zip code name in the month of March.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code creates a color coded map of MD that shows how many foreclosures there were in each zip code designation in March. The lighter the color, the more foreclosures.
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer** Libraries to tidy the census, as well as census info, has been loaded into this data notebook.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We have one chart that tracks foreclosure notices by county, and another chart that shows the MD population by county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This chart shows the population in each MD county in 2021.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We have two tables that show the population and foreclosure notices.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This table shows a better, more tidy table with populations in each MD county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This table shows foreclosures in each MD county, as well as the population of each county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** PG County has the most foreclosures, followed by Baltimore County.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code creates a color-coded map that shows the foreclosure rate of MD counties. The darker blue, the lower foreclosure rate.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This is a more aesthetically pleasing map that shows foreclosure data in each county. The light yellow shows high rates, dark black shows low rates.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** It seems like Garrett County is an outlier, in the sense that the two counties next to it have high foreclosure rates, but Garrett has extremely low rates. Most of MD falls into a purple/orange category, so they have somewhat high foreclosure rates but nothing crazy. There is only one county with a very high foreclosure rate -- Charles County.
